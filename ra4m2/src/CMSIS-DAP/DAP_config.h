...
__STATIC_FORCEINLINE void     PIN_TDI_OUT (uint32_t bit) {
    R_BSP_PinWrite(PIN_CMSIS_DAP_TDI, bit & 0x000000001 ? BSP_IO_LEVEL_HIGH : BSP_IO_LEVEL_LOW);
}
...
__STATIC_FORCEINLINE uint32_t PIN_TDO_IN  (void) {
  ...
  val = R_BSP_PinRead(PIN_CMSIS_DAP_TDO);
  return val;
}
...
__STATIC_FORCEINLINE uint32_t PIN_nTRST_IN   (void) {
  ...
  val = R_BSP_PinRead(PIN_CMSIS_DAP_NTRST);
  return val;
...
__STATIC_FORCEINLINE void     PIN_nTRST_OUT  (uint32_t bit) {
    R_BSP_PinWrite(PIN_CMSIS_DAP_NTRST, bit & 0x000000001 ? BSP_IO_LEVEL_HIGH : BSP_IO_LEVEL_LOW);
}
...
__STATIC_INLINE void LED_RUNNING_OUT (uint32_t bit) {
    ...
    int32_t ledindex = LED_INDEX_RUNNING;
    if (ledindex >= 0 && ledindex < g_bsp_leds.led_count)
    {
      R_BSP_PinWrite((bsp_io_port_pin_t)g_bsp_leds.p_leds[ledindex], bit ? BSP_IO_LEVEL_HIGH : BSP_IO_LEVEL_LOW);
    }
